...
def run_resnet_inference(
    ...
    print("run_resnet_inference start")
    disable_persistent_kernel_cache()
    disable_compilation_reports()
...
def test_demo_sample(device, use_program_cache, batch_size, input_loc, imagenet_label_dict, model_location_generator):
    '''
        imagenet_label_dict:        read from "model/sample_data/imagenet_class_labels.txt"
        device:                     return from $TT_METAL_HOME/conftest.py
        model_location_generator:   return from $TT_METAL_HOME/conftest.py
