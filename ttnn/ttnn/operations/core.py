...
def __getitem__(input_tensor: ttnn.Tensor, slices) -> ttnn.Tensor:
    ...
    logger.debug(f"input_rank = {input_rank}")
    ...
    logger.debug(f"normalized slices = {slices}, len(slices) = {len(slices)}")
    ...
    if input_rank <= 4:
        logger.debug(f"Before unqueeze_to_4D input_tensor.shape = {input_tensor.shape}")
        input_tensor = ttnn.unsqueeze_to_4D(input_tensor)
        logger.debug(f"After unqueeze_to_4D input_tensor.shape = {input_tensor.shape}")

        while len(slices) != 4:
            slices = (slice(None, None, None),) + slices

        logger.debug(f"After padding slices = {slices}")
        ...
        logger.debug("slice_start = {}, slice_end = {}".format(slice_start, slice_end))
        ...
        logger.debug("padded_slice_end = {}".format(padded_slice_end))
        logger.debug(f"input_tensor.shape.with_tile_padding(): {input_tensor.shape.with_tile_padding()}")

        if list(padded_slice_end) == list(input_tensor.shape.with_tile_padding()):
            output = input_tensor
            logger.debug("output = input_tensor")
        else:
        ...
        logger.debug("output.shape = {}, output_shape = {}, padded_output_shape = {}".format(output.shape, output_shape, padded_output_shape))
